...
| `--jsonpath PATH`  | Extract specific JSON elements using JSONPath (e.g., `"$.results[*].note_slug"`). Eliminates JSON envelope overhead entirely. Dialect (PaesslerAG/jsonpath v0.1.1, verified): dotted paths (`.key`), array wildcards (`[*]`), numeric indices (`[0]`), slices (`[0:2]`), and recursive descent (`$..key`) all work; bracket-quoted keys (`$['key']`), filter expressions (`[?(...)]`), and jq-style pipes do NOT. The result of a wildcard is a JSON array; a missing key is an error (exit 1). Path shorthand is normalized: `results[*].note_slug` or `$.results[*].note_slug` and `{.results[*].note_slug}` (jq-style braces) are all accepted. For multi-field extraction use `--format tsv` or two `--jsonpath` calls. | —       |
