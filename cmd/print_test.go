...
func TestFilterResults_IncludeText(t *testing.T) {
	...
	globals := &Globals{}
	flagsFalse := &ChunkDisplayFlags{IncludeText: false}
	filtered := filterResults(results, globals, flagsFalse)
	if len(filtered) != 1 {
	...
	globalsTrue := &Globals{}
	flagsTrue := &ChunkDisplayFlags{IncludeText: true}
	filteredTrue := filterResults(results, globalsTrue, flagsTrue)
