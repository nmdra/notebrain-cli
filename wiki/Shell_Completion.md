...
| Enum values (`--format`) | `text`, `json`, `tsv` |
| Enum values (`--log-level`) | `debug`, `info`, `warn`, `error` |
...
- **Slug completion is empty**: make sure that the index is populated with `notebrain stats`.
